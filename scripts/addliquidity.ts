...
const main = async () => {
    ...
    const approveTx = await USDC.connect(impersonatedSigner).approve(ROUTER , amount1);
    await approveTx.wait();

    const approveTx2 = await DAI.connect(impersonatedSigner).approve(ROUTER , amount2);
